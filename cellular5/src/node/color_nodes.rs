...
pub enum FloatColorNodes {
    ...
    Grayscale { alpha: UNFloat, child: NodeBox<UNFloatNodes> },
...
pub enum HSVColorNodes {
    ...
    #[mutagen(gen_weight = branch_node_weight)]
    DucksFractal {
        child_offset: NodeBox<SNComplexNodes>,
        child_unfloat_scale: NodeBox<SNFloatNodes>,
        child_nibble_scale: NodeBox<NibbleNodes>,
    ...
    fn compute(&self, mut compute_arg: ComArg) -> Self::Output {
        ...
        match self {
            ...
            DucksFractal {
                ...
                let offset = child_offset.compute(compute_arg.reborrow());
                let unfloat_scale = child_unfloat_scale.compute(compute_arg.reborrow());
                let nibble_scale = child_nibble_scale.compute(compute_arg.reborrow());
                let iterations = child_iterations.compute(compute_arg.reborrow()).into_inner() / 2;

                let c = Complex::new(
                    f64::from(
                        compute_arg.coordinate_set.y.into_inner() * nibble_scale.into_inner() as f32 * unfloat_scale.into_inner(),
                    ),
                    f64::from(
                        compute_arg.coordinate_set.x.into_inner() * nibble_scale.into_inner() as f32 * unfloat_scale.into_inner(),
                    ),
                );

                let c_offset = Complex::new(
                    f64::from(
                        offset.re().into_inner(),
                    ),
                    f64::from(
                        offset.im().to_unsigned().into_inner() * -1.0 * PI,
                    ),
                );
            ...
            DucksFractal {
                ...
                let c = Complex::new(
                    f64::from(
                        ...
                        nibble_scale.into_inner() as f32 * 
                        unfloat_scale.into_inner(),
                    ),
                    f64::from(
                ...
                // let c = Complex::new(
                //     f64::from(
                //         UNFloat::new_sin(compute_arg.coordinate_set.t / 256.0).into_inner() * 
                //         compute_arg.coordinate_set.y.into_inner(),// * nibble_scale.into_inner() as f32 * unfloat_scale.into_inner(),
                //     ),
                //     f64::from(
                //         UNFloat::new_sin(compute_arg.coordinate_set.t / 256.0).into_inner() * 
                ...
                // let mut c_offset = Complex::new(
                //     f64::from(SNFloat::new_sin(compute_arg.coordinate_set.t / 2000.0).into_inner()),
                //     f64::from(0.001 * compute_arg.coordinate_set.t * PI)
                // ).exp();

                // c_offset.im = c_offset.im * f64::from(-1.0 * PI);
